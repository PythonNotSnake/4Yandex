...
def telegram_bot_sendtext(ChatIDs, bot_message):
    ...
    session.proxies['http'] = '104.236.47.73:8080'
...
def RoutesSender():
    ...
    sql="""select DateStart from [DBReport].[dbo].[RoutesLogMorning] where cast(DateStart as date)=cast(getdate() as date) and datepart(HOUR,DateStart)=8 and StoreMailStatus>0"""  
    cursor.execute(sql)  
    data =  cursor.fetchall()
    db.commit()
    if len(data)==0:
...
Похоже что-то не так. До сих пор не отправлено не одного письма. Скорее всего это ошибка (возможен случай, когда нет маршрутов для рассылки)."""
    ...
    sql="""declare @today date =cast(getdate() as date)
            ...
            from [DBReport].[dbo].[RoutesLogMorning] 
...
token='860957767:AAFNyG3RTV5eW3OnRSPXuDeQ'  
